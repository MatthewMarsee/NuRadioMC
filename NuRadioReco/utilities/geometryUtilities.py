...
#obtain refracted zenith based on antenna depth 
#currently implemented for plane wave (fixed ray path/launch angle)
def get_refracted_zenith_ray(ray,antenna_depth):
    #ray should be from .h5 file generated by rayreader scripts 
    assert len(ray)==3
    r = ray[0,:]
    z = ray[1,:]

    i = np.argmin(np.abs(z - antenna_depth))

    if z[i] == antenna_depth:
        if i == len(z) - 1:
            i -= 1
        r1, r2 = r[i], r[i+1]
        z1, z2 = z[i], z[i+1]
    else:
        # Determine which segment contains the crossing
        if z[i] > antenna_depth:
            i1 = i
            i2 = i + 1
        else:
        ...
        if i1 < 0 or i2 >= len(z):
            raise ValueError("Target depth outside ray range")

        r1, r2 = r[i1], r[i2]
        z1, z2 = z[i1], z[i2]

    dr = r2 - r1
    dz = z2 - z1   # should be negative

    return np.arctan2(abs(dr), -dz)
...
#decouple transmission coefficients from antenna zenith
def get_surface_transmission(zenith_surface, n_air=1.0,n_ice=1.74):
    #n_ice = ice_model.get_index_of_refraction([0,0,-0.01])

    t_theta = get_fresnel_t_p(zenith_surface, n_ice, n_air)
    t_phi   = get_fresnel_t_s(zenith_surface, n_ice, n_air)

    return t_theta, t_phi
...
def fresnel_factors_and_signal_zenith(detector, station, channel_id, zenith):
    ...
    * No refraction is simulated if non of the aferomentioned conditions apply.
    ...
    #if position less than 3m
    #   Load ray & get_zenith function
    #   obtain zenith angle based on antenna depth

...
def get_efield_antenna_factor(station, frequencies, channels, detector, zenith, azimuth, antenna_pattern_provider, ray_path, efield_is_at_antenna=False):
    """
    ...
    t_theta, t_phi = geo_utl.get_surface_transmission(zenith) #does not need to be done per channel for PLANE WAVE
    for iCh, channel_id in enumerate(channels):
        if not efield_is_at_antenna:
            antenna_position = detector.get_relative_position(station.get_id(), channel_id)
            antenna_depth = antenna_position[2]

            #refracted source zenith angle, as seen at each antenna depth
            zenith_antenna = geo_utl.get_refracted_zenith_ray(ray_path, antenna_depth)
            t_theta, t_phi = geo_utl.get_surface_transmission(zenith_antenna)
